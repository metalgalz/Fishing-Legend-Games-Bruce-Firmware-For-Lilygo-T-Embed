...
function main() {
  ...
  while(true) {
    ...
    if (appState === 0) {
        ...
        for(var y=skyH+10; y<dh-dkH; y+=12) { 
             var rowSeed = (y * 0.1); 
             var rowOff = Math.sin(waveTime + rowSeed) * 5; // Movement
             
             for(var x=0; x<gW; x+=16) { 
                 var wx = M(x + rowOff);
                 if (wx > 0 && wx < gW - 4) {
                     // Zigzag V shape
                     spr.drawPixel(wx, y, waveCol);
                     spr.drawPixel(wx+1, y+1, waveCol);
